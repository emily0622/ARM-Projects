...
PRIV_TIMER_CHECK:
    			//....[FILL IN CODE HERE ] ADDED
				CMP R5, #MPCORE_PRIV_TIMER_IRQ
				BNE UNKNOWN_IRQ
				BL PRIV_TIMER_ISR//KEYS_CHECK //TIMER_ISR  LITERALLY NO IDEA
				B EXIT_IRQ
UNKNOWN_IRQ:
				B UNKOWN_IRQ  //added
...
KEYS_CHECK:
    			//... [FILL IN CODE HERE ] ADDED
				CMP R5, #KEYS_IRQ
				BNE UNKNOWN_IRQ//PRIV_TIMER_CHECK
				BL KEY_ISR
				B EXIT_IRQ
...
TOCENTRE:		//ADDED
				PUSH {R10, R11}
				AND R10, R3, #0b0000011111
				AND R11, R3, #0b1111100000
				LSL R10, #1
				LSR R11, #1
				MOV R3, #0 //reset pattern
				ADD R3, R10
				ADD R3, R11

				CMP R11, #0b0000100000
				POP {R10, R11}
				BEQ C_O
				B DONE_SWEEP
...
CHK_KEY3:		//ADDED
				...
				//CMP R1, #0b1000
				//BNE CONTINUE
				//MOV R8, #1 //KEY3 HAS BEEN PRESSED
...
START_STOP:		LDR		R0, =MPCORE_PRIV_TIMER		// timer base address
					...
					//ADDED THESE TWP LINES IDK THO
					MOV R8, #1
					AND R7, R7, #0b1 //KEY ENABLE BIT
					EOR R7, R7, #1 //XOR ENABLE AND 1 TO TOGGLE
					AND R1, R7, #0b110 
					STR	R1, [R0, #0x8]	
...
*/
			.global	_start
_start:
			//... initialize the IRQ stack pointer ... ADDED
			MOV R0,#0b10010 //IRQ mode
			MSR CPSR, R0
			LDR SP, =#0x20000 //SP for IRQ
			//... initialize the SVC stack pointer ...
			MOV R0, #0b10011 //SVC mode
			MSR CPSR, R0
			LDR SP, =0x10000 //SP for SVC mode

			BL			CONFIG_GIC				// configure the ARM generic interrupt controller
			BL			CONFIG_PRIV_TIMER		// configure the MPCore private timer
			BL			CONFIG_KEYS				// configure the pushbutton KEYs
			
			//... enable ARM processor interrupts ... ADDED
			//LDR R0, =KEY_BASE
			//MOV R1, #0b1000
			//STR R1, [R0, #8] //enable KEY3 interrupt
			
			//LDR R0, =MPCORE_PRIV_TIMER
			//LDR R1, =100000000
			//STR R1, [R0]
			//MOV R1, #0b111
			//STR R1, [R0, #8]//enable and interrupts
			MOV R0, #0b00010011//enable interrupts in SVC mode
			LDR		R6, =0xFF200000 		// red LED base address
			MOV R8, #0 //for key purposes
//LOOP:
//	B LOOP //IDK IF I SHOULD DO THIS ,THE WHOLE FUNCT IS MINE
...
CONFIG_PRIV_TIMER:
			...
			//... code not shown ADDED!!!
			LDR R1, =20000000//0.1 s
			STR R1, [R0]
			MOV R1, #0b111 //I=1, A=1, E=1
			STR R1, [R0, #8] //write control to reg			
...
CONFIG_KEYS:
			...
			//... code not shown ADDED
			MOV R1, #0b1000
			STR R1, [R0, #8] //enable KEY3 interrupts
